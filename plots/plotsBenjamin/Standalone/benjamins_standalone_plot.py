...
h_fake_dxy = ROOT.TH1D("Fake #mu d_{xy}", "Fake #mu d_{xy}", 100, -5, 5)
h_fake_relIso = ROOT.TH1D("Fake #mu relIsoR03", "Fake #mu relIsoR03", 100, -5, 5)
h_fake_absIso = ROOT.TH1D("Fake #mu absIsoR03", "Fake #mu absIsoR03", 100, -5, 5)

hist_stop = ROOT.TH1D("#mu d_{xy} directly from susy", "#mu d_{xy} directly from susy",100, -5, 5)
hist_stop_nonprompt = ROOT.TH1D("#mu d_{xy} from susy, nonprompt", "#mu d_{xy} from susy, nonprompt", 100, -5, -5)
hist_prompt = ROOT.TH1D("#mu d_{xy} prompt", "#mu d_{xy} prompt", 100, -5, 5)
hist_notprompt = ROOT.TH1D("#mu d_{xy} notprompt", "#mu d_{xy} notprompt", 100, -5, 5)
...
def makehisto(list_hists, title, outputname, xlabel, ylabel):
    ...
    legend.Draw("SAME")
    canv.Print("/users/benjamin.wilhelmy/StandealonePlotting/plots/{}".format(outputname)+".png")
...
if __name__ == "__main__":
    ...
    makehisto(relIso_histos, "relIso dR03 all for #mu with p_{t} #geq 20 MeV", "relIso_{}".format(maxiter), "relIsodR03_all for #mu", "Nr. Events")

    absIso_histos = [h_stop_absIso, h_stop_nprom_absIso, h_prompt_absIso, h_notprompt_absIso, h_fake_absIso]
    makehisto(absIso_histos, "absIso dR03 all for #mu with p_{t} < 20 MeV", "absIso_{}".format(maxiter), "p_{t} * relIsodR03_all for #mu", "Nr.Events")
    ...
    makehisto(relIso_04_histos, "relIso dR04 all for #mu with p_{t} #geq 20 MeV", "relIso_dR04_{}".format(maxiter), "relIsodR04_all for #mu", "Nr. Events")
    ...
    makehisto(absIso_04_histos, "absIso dR04 all for #mu with p_{t} < 20 MeV", "absIso_dR04_{}".format(maxiter), "p_{t} * relIsodR04_all for #mu", "Nr. Events")
