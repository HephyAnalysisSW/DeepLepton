...
argParser.add_argument('--flatSample',         action='store',           default='TTs_Muons_2016')
argParser.add_argument('--refTraining',        action='store',     choices=['final_deepLepton', 'initial_deepLepton','no'],      default='no')
argParser.add_argument('--name',               action='store',           default='')
argParser.add_argument('--smallPlotSize',      action='store_true',                help='Plot a smaller graph?', )
...
if args.flat:
    ...
    #DeepLepton reference training
    ref_flatSample="TTs_Muons_2016" if args.refTraining=="initial_deepLepton" else "TTs_Muons_biLSTM_splitDense_elu_2016"
    ref_flat_sampleInfo = vars()[ref_flatSample]
    ref_flat_files, ref_predict_files = get_flat_files( ref_flat_sampleInfo['flat_directory'], ref_flat_sampleInfo['predict_directory' if args.testData else 'predict_directory_trainData'])
    ref_flat_sample = get_flat_sample( ref_flat_sampleInfo['training_name'], ref_flat_sampleInfo['sample_name'], ref_flat_files, ref_predict_files )

    ref_sig_sample = ref_flat_sample
    ref_bkg_sample = ref_flat_sample
...
#latex plot info
if flat_sampleInfo["train_data"]=="TTs muons":
    traindata_latex="t#bar{t}"
elif flat_sampleInfo["train_data"]=="TTJets muons":
    traindata_latex="t#bar{t}Jets"
elif flat_sampleInfo["train_data"]=="DYvsQCD+TTs muons":
    traindata_latex="DY+QCD and t#bar{t}"
else:
    traindata_latex="DY+QCD"

if flat_sampleInfo["test_data"]=="TTs muons":
    testdata_latex="t#bar{t}"
elif flat_sampleInfo["test_data"]=="TTJets muons":
    testdata_latex="t#bar{t}Jets"
elif flat_sampleInfo["test_data"]=="DYvsQCD+TTs muons":
    testdata_latex="DY+QCD and t#bar{t}"
else:
    testdata_latex="DY+QCD"
...
kinematic_selection_latex       = '#font[72]{p}_{T} > '+str(args.ptMin)+' GeV' if args.ptMax==0 else str(args.ptMin)+' GeV < #font[72]{p}_{T} < '+str(args.ptMax)+' GeV'
selection_latex                 = "loose ID "+("muons" if args.flavour=="muo" else "electrons")
sampleInfo_latex                = testdata_latex+" events (2016)"
training_sampleInfo_latex       = traindata_latex+" events, muons (#font[52]{p}_{T} > 5 GeV)"
small_training_sampleInfo_latex = traindata_latex+" events, muons (#font[52]{p}_{T} > 15 GeV)"
...
if args.name in ['','final']:
    addcolor=2
elif args.name in ['initial']:
    addcolor=0
else:
    addcolor=1
# lepton Ids
...
for lepton_id in lepton_ids:
    ...
    if lepton_id in [initial_deepLepton, final_deepLepton]:
        ref                    = ref_sig_sample.getYieldFromDraw( selectionString = selectionString, weightString = weightString) 
        lepton_id["sig_h_eff"] = ref_sig_sample.get1DHistoFromDraw(     lepton_id["var"], lepton_id["thresholds"], selectionString = selectionString, weightString = weightString, binningIsExplicit = True )
    else:
    ...
    print selectionString
    if lepton_id in [initial_deepLepton, final_deepLepton]:
        ref                    = ref_bkg_sample.getYieldFromDraw( selectionString = selectionString, weightString = weightString )
        lepton_id["bkg_h_eff"] = ref_bkg_sample.get1DHistoFromDraw( lepton_id["var"], lepton_id["thresholds"], selectionString = selectionString, weightString = weightString, binningIsExplicit = True )
    else:
...
c = ROOT.TCanvas()
c.SetCanvasSize(575 if args.smallPlotSize else 650,500)
option = "L"
same    ="A"
for lepton_id in lepton_ids:
...
c.BuildLegend(0.69,0.41,0.89,0.57)
...
def drawObjects():
    ...
    tex.DrawLatex(*line)
    line = (0.12, 0.835, kinematic_selection_latex)
    tex.DrawLatex(*line)
    ...
    tex.SetTextFont(42)
    tex.SetTextAlign(11)
    line = (0.10, 0.92, ("training info: {trainData} {ref}".format( trainData= small_training_sampleInfo_latex if args.smallPlotSize else training_sampleInfo_latex, ref="" if args.smallPlotSize else " -  "+flat_sampleInfo['training_name'] )))
    tex.DrawLatex(*line)
